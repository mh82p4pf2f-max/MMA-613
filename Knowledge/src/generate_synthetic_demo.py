"""
generate_synthetic_demo.py — SYNTHETIC DEMONSTRATION DATA ONLY.

WHY THIS EXISTS
---------------
The real governed panel (`Knowledge/processed/policy_triage_panel.csv`) is honest
about missingness: four context columns (low_income_rate, housing_pressure_proxy,
income_value, population) are NaN because their StatCan source tables have not been
downloaded, so confidence is capped at MEDIUM. Per CLAUDE.md those NaNs must NEVER
be filled in the real panel — inventing a value there is an automatic fail.

This script builds a SEPARATE, CLEARLY-LABELLED synthetic dataset so the dashboard
and the gold-case output (per-group rows such as "Youth 15-24, Alberta") can be
demonstrated end-to-end before the real context tables are connected. It writes to
`Knowledge/synthetic/` — never to `Knowledge/processed/`.

WHAT IS REAL vs SIMULATED
-------------------------
REAL  (from StatCan LFS 14-10-0287-01, via labour_force_clean.csv):
    employment_rate, participation_rate, unemployment_rate (and derived youth rate
    + trailing-12-month changes) for every geo x age_group x gender x month.
SIMULATED (deterministic, seeded; geo/age/gender/year-anchored to plausible ranges):
    low_income_rate, housing_pressure_proxy, income_value, population.

Every output row carries `is_synthetic=True` and a `dataset_provenance` string, and
the score_explanation is suffixed with a synthetic-demo marker. The score itself is
computed by the REAL scoring.py so it stays consistent with the production pipeline.

GOVERNANCE: nothing here is a measurement of reality. Do not merge into the real
panel, do not present as StatCan-sourced, do not use for any program decision.
"""
...
import scoring  # the REAL scoring module — reused so synthetic scores match production

CLEAN = os.path.join(ROOT, "Knowledge", "processed", "labour_force_clean.csv")
OUT_DIR = os.path.join(ROOT, "Knowledge", "synthetic")
PANEL_OUT = os.path.join(OUT_DIR, "policy_triage_panel_SYNTHETIC.csv")
LONG_OUT = os.path.join(OUT_DIR, "synthetic_statcan_long_2015_2026.csv")
...
PROV_FILL = "SYNTHETIC — simulated for demo; no real StatCan table downloaded"

# ----------------------------------------------------------------------------
# Anchors for the SIMULATED context columns. These are plausible orders of
# magnitude, NOT measured values. They are deterministic given the seed.
# ----------------------------------------------------------------------------
...
def national_shelter_curve():
    """Monthly shelter-CPI YoY % path 2015-2026 (national). Hand-shaped to mirror
    the well-known low-then-spike-then-ease pattern; clearly a synthetic proxy."""
...
# ----------------------------------------------------------------------------
# Step 1 — load REAL LFS rates and pick the data_type with best demographic coverage
# ----------------------------------------------------------------------------
...
def build_wide_panel(df, data_type):
    ...
    for c in ["employment_rate", "participation_rate", "unemployment_rate"]:
        if c not in wide.columns:
            wide[c] = np.nan
    # keep only rows with all three core REAL rates (so the demo is complete)
    before = len(wide)
    wide = wide.dropna(subset=["employment_rate", "participation_rate", "unemployment_rate"])
    print(f"[panel] kept {len(wide):,} of {before:,} intersection-months with all 3 real rates")
    return wide.reset_index(drop=True)


# ----------------------------------------------------------------------------
# Step 2 — youth rate + trailing-12-month change features (derived from REAL rates)
# ----------------------------------------------------------------------------
...
def add_youth_and_changes(wide):
    ...
    # mirroring the real panel's "youth as a cross-cutting driver" design.
    ...
    # Matches the real pipeline (AI Council 2026-06-27): unemployment 12-mo;
...
# ----------------------------------------------------------------------------
# Step 3 — SIMULATE the four context columns (deterministic, anchored, flagged)
# ----------------------------------------------------------------------------

def simulate_context(wide):
...
# ----------------------------------------------------------------------------
# Step 4 — score with the REAL scoring.py, then assemble panel + provenance
# ----------------------------------------------------------------------------

PROVENANCE = ("SYNTHETIC DEMO — LFS rate columns derived from real StatCan "
              "14-10-0287-01; low_income_rate, housing_pressure_proxy, income_value "
              "and population are SIMULATED (not measured). Not for program decisions.")
...
def assemble(wide):
    ...
    # synthetic-demo marker on every explanation so it can't be mistaken for a finding
    scored["score_explanation"] = scored["score_explanation"] + \
        " [SYNTHETIC DEMO DATA — illustrative only, not a real measurement.]"

    scored["is_synthetic"] = True
    scored["dataset_provenance"] = PROVENANCE
    scored["labour_force_data_frequency"] = "monthly"
    scored["housing_data_frequency"] = "monthly (SIMULATED)"
    scored["income_data_frequency"] = "annual (SIMULATED, repeats across the year)"
    scored["population_data_frequency"] = "annual (SIMULATED, repeats across the year)"
    scored["source_product_id_lfs"] = "14100287"
    scored["source_table_lfs"] = "14-10-0287-01 Labour force characteristics (LFS) — REAL rates"
    scored["source_product_id_income"] = "SYNTHETIC"
    scored["source_table_income"] = PROV_FILL
    scored["source_product_id_housing"] = "SYNTHETIC"
    scored["source_table_housing"] = "SYNTHETIC — simulated shelter-CPI proxy (not CPI 18-10-0004)"
    scored["source_product_id_population"] = "SYNTHETIC"
    scored["source_table_population"] = "SYNTHETIC — simulated population estimate (not 17-10-0005)"
    ...
    cols = [
        ...
        "is_synthetic", "dataset_provenance",
...
# ----------------------------------------------------------------------------
# Step 5 — large LONG-format synthetic StatCan-style extract (the volume bridge)
# ----------------------------------------------------------------------------

def build_long_extract(df_real_rates, panel):
    """Tidy long extract at real LFS granularity. Real rate rows (both data_types)
    plus simulated count + context indicators. No invented geography or individuals."""
    rows = []

    # 1) REAL rate rows (Unemployment/Employment/Participation rate), both data_types
    ...
    real["source_table"] = "14-10-0287-01 LFS (REAL StatCan rate)"
    real["is_synthetic"] = False
    rows.append(real)

    # 2) SIMULATED LFS count indicators, derived from the panel slice (Unadjusted-like).
    #    population × rate gives plausible person counts (in thousands, like LFS).
    ...
    # as Unadjusted — labelling them SA would make them contradict the real SA rate
    # rows in this same file. The accounting identities (LF = Emp + Unemp;
    ...
    for name, series in count_map.items():
        ...
        blk["source_product_id"] = "SYNTHETIC"
        blk["source_table"] = "SYNTHETIC — simulated LFS-style count (Unadjusted basis)"
        blk["is_synthetic"] = True
        rows.append(blk)

    # 3) SIMULATED context indicators (annual + monthly proxy)
    ctx = panel[["ref_date", "year", "month", "geo", "gender", "age_group",
                 "low_income_rate", "income_value", "housing_pressure_proxy"]].copy()
    ctx_specs = [
        ("Low-income rate (LIM-AT)", "low_income_rate", "Percent", "annual",
         "SYNTHETIC — simulated low-income proxy (not 11-10-0135)"),
        ("Median employment income", "income_value", "Dollars", "annual",
         "SYNTHETIC — simulated income (not 11-10-0239 / 98-10-0597)"),
        ("Shelter CPI (YoY %)", "housing_pressure_proxy", "Percent", "monthly",
         "SYNTHETIC — simulated shelter-CPI proxy (not 18-10-0004)"),
    ]
    ...
    long = long[["ref_date", "year", "month", "geo", "gender", "age_group", "indicator",
                 "data_type", "value", "unit", "frequency", "is_synthetic",
...
def main():
    ...
    print("=" * 78)
    print("SYNTHETIC DEMO DATA GENERATION — writes ONLY to Knowledge/synthetic/")
    print("=" * 78)

    df, chosen_dt = load_real_rates()
    wide = build_wide_panel(df, chosen_dt)
    wide = add_youth_and_changes(wide)
    wide = simulate_context(wide)
    panel = assemble(wide)

    # --- regression guards: fail loudly rather than ship a governance-broken file ---
    assert set(panel["geo"]).issubset(set(PROVINCES)), "non-province geography leaked in!"
    assert bool(panel["is_synthetic"].all()), "a row is not flagged synthetic!"
    ...
    print("[panel] regression guards passed (geo, synthetic flag, annual-constancy, escalation)")
    ...
    print(f"[long] real vs synthetic rows:\n{long['is_synthetic'].value_counts()}")
