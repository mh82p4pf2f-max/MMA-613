...
- **Score** a transparent 0–100 `policy_review_priority_score` per geography × group
  (`Knowledge/src/scoring.py`): **low-income rate (0.25 — largest factor)**, unemployment level (0.22)
  & 12-mo change (0.13), employment decline vs 3-mo avg (0.10), participation decline vs 3-mo avg
  (0.10), youth unemployment (0.10), housing-pressure proxy (0.10). Change windows: unemployment is 12-mo;
...
- **Next build step — the fully functional app:** build out `dashboard/app.py` on the synthetic demo
  panel (the nine views — **no in-app synthetic label, per owner direction 2026-06-27**), wiring the
  three capabilities, with the **real** governed panel still the authority via `verify.py`. Then rebuild
  the real panel to score age/gender intersections as their own rows (e.g. "Youth 15–24, Alberta") so
  the gold case becomes live on real data.
...
| 2026-06-27 | **Dashboard redesign + owner decision on the synthetic label.** Restyled `app.py` to a calm "government data room" look (navy/slate theme via `.streamlit/config.toml`, white cards, cleaner tabs) aimed at a federal analyst: a "Start here" landing names the first area to review in a hero card, with a **Claude-drafted cycle briefing** (cycle-briefing-writer format, grounded in the panel — no invented figures) and consistent confidence chips. **Owner override (2026-06-27):** the full-page `SYN_BANNER` and all in-app synthetic labels (masthead badge, "🧪 SYNTHETIC" captions, the `[SYNTHETIC DEMO DATA]` tag shown in score explanations) were **removed at owner direction** — the app no longer marks the data as synthetic in its UI. The underlying data is still synthetic; the no-invented-values rule, the real-panel-untouched rule, and `README_SYNTHETIC.md` **remain in force** (the data files still carry `is_synthetic` / provenance). Still prototype — not Council-reviewed. |
| 2026-06-27 | **Reweighted the score to make income a primary factor (owner direction).** `scoring.py` WEIGHTS changed: low-income **0.15 → 0.25** (now the largest single factor), unemployment level **0.30 → 0.22**, 12-mo change **0.15 → 0.13** (others unchanged; sum still 1.00). Both panels re-scored from their component columns via `scoring.add_scores()` — real panel (6,655 rows) and synthetic demo. `verify.py` re-run: **PASS**. Consequence: on the real panel low-income is still NaN, so the heavier income weight is unbacked → confidence dropped (now mostly **medium ~0.65**, 66 early-series rows fall to **low**); connecting 11-10-0135-01 would lift it back. Note: this nudges the framing back toward "economic distress" vs strict labour-market — **flag for AI Council** (weights are Council-reviewable). App score text + CLAUDE.md weight refs updated. Prototype — not Council-ratified. |
| 2026-06-27 | **Restructured the app for clarity + full data use** (dashboard-product-designer skill). Nine tabs → **four views** (Start here · Why—the drivers · Score & evidence · Governance & caveats); income/affordability folded into "drivers" as expanders rather than standalone tabs. App now loads the **full per-group panel** and adds a **Provinces / Demographic-groups toggle** (age band + gender selectors) so the gold case ("Youth 15–24, Newfoundland") ranks as its own row — previously the app collapsed everything to province × 15+ × Total-Gender. Canada excluded from the focus ranking (national baseline). `verify.py` re-run: **PASS**, all 6,655 real-panel rows recompute, pending columns still NaN. The 450k-row `synthetic/synthetic_statcan_long_2015_2026.csv` is intentionally not loaded by the app (redundant with the panel; volume/pipeline-test artifact only). Still prototype — not Council-reviewed. |
